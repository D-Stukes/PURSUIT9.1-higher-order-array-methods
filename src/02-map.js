...
function getSongTitles(songs) {
  let allSongTitles = exampleSongData.map((song) => song.title)
...
function getSongDetails(songs) {
  let allSongTitlesAndArtists = songs.map((song) => song.title +  " by " + song.artist)
  return allSongTitlesAndArtists
}
...
function getTitleAndArtist(songs) {
  const record = songs.map( (hit) => {
    let newObj = {}
    newObj[hit.title] = hit.artist
    return newObj
  })
  return record
}
